...
TagProbeFitter::TagProbeFitter(const std::vector<std::string>& inputFileNames, string inputDirectoryName, string inputTreeName, string outputFileName, int numCPU_, bool saveWorkspace_, bool floatShapeParameters_, const std::vector<std::string>& fixVars_) {
	...
	hpvalue = new TH1F("hpvalue", "List of p-values for fits", 360, 0, 360);
	OtaCounter = 1;
...
void TagProbeFitter::saveFitPlot(RooWorkspace* w) {
	...
	hpvalue->SetBinContent(OtaCounter, pVlu);
	...
	for (unsigned int i = lowSgn; i < highSgn; i++) {
		...
		Chi2 = Chi2 + pow(ypullfail[i-1], 2);
		hPullFail->SetBinContent(i, ypullfail[i-1]);
	}
	UnNormChi2 = Chi2;
	...
	hpvalue->SetBinContent(120+OtaCounter, pVlu);
	...
	for (unsigned int i = lowSgn; i < highSgn; i++) {
		...
		Chi2 = Chi2 + pow(ypullall[i-1], 2);
		hPullAll->SetBinContent(i, ypullall[i-1]);
	}
	UnNormChi2 = Chi2;
	Dof = nFullBinsPull - nFitParam;
	...
	hpvalue->SetBinContent(240+OtaCounter, pVlu);
	OtaCounter++;
